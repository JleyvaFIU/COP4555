...
let rec unify (t1, t2) =
  ...
  | (t, VARIABLE a)    -> unify (VARIABLE a, t)
  | (INTEGER, INTEGER) -> I
  | (BOOLEAN, BOOLEAN) -> I
...
let rec upfrom n =
  ...
  Cons (VARIABLE (List.nth letters (n % 26) + if n/26 = 0 then "" else string (n/26)),
        fun () -> upfrom (n+1))
...
let (newtypevar, reset) =
  let vars = ref alltypevars
  let hd (Cons (x, xsf)) = x
  let tl (Cons (x, xsf)) = xsf()
  ((fun () -> let next = hd (!vars) in vars := tl (!vars); next),
   (fun () -> vars := alltypevars))
